...
<div class="container py-4">
    <input type="hidden" id="autoOpenFeedback" value="{{ session('open_feedback') ? '1' : '0' }}" />
    
    {{-- Notifikasi --}
    @if(session('success'))
        <div class="alert alert-success border-0 rounded-4 shadow-sm d-flex align-items-center mb-4">
    ...
    <div class="action-header-bar">
        <div>
            <div class="d-flex align-items-center gap-3">
                <a href="{{ route('admin.aspirasi.index') }}" class="btn btn-back" aria-label="Kembali ke daftar aspirasi" title="Kembali ke daftar">
                    <i class="fas fa-arrow-left"></i>
                </a>
                <div>
                    <span class="text-muted small fw-bold">ID TIKET</span>
                    <h5 class="mb-0">#{{ $aspirasi->id_aspirasi }}</h5>
                </div>
            </div>
        </div>
        <div class="status-pill-large status-{{ $aspirasi->status }}">
            <i class="fas fa-circle-fill" style="font-size: 8px;"></i> {{ $aspirasi->status }}
        </div>
    </div>
    ...
    <div class="row g-4">
        ...
        <div class="col-lg-8">
            <div class="bento-card">
                ...
                <div class="ux-tracker">
                    ...
                    <div class="tracker-step {{ in_array($aspirasi->status, ['Diproses', 'Selesai']) ? 'active completed' : '' }}">
                        <div class="step-icon"><i class="fas fa-sync"></i></div>
                        <span class="small fw-bold">Diproses</span>
                    </div>
                    <div class="tracker-step {{ $aspirasi->status == 'Selesai' ? 'active completed' : '' }}">
                ...
                <div class="mb-4">
                    <span class="badge">
                        <i class="fas fa-tag me-2"></i> {{ $aspirasi->nama_kategori }}
                    </span>
                </div>
                ...
                {{-- Bukti Visual: Premium Image Section --}}
                <div class="mt-5">
                    <h6 class="fw-bold text-uppercase"><i class="fas fa-image me-2"></i>Lampiran Bukti</h6>
                    @php $fotoPath = $aspirasi->foto_aspirasi ?? $aspirasi->foto ?? null; @endphp
                    @if(!empty($fotoPath))
                        <div class="image-wrapper rounded-3" data-bs-toggle="modal" data-bs-target="#imageFullModal" style="max-width: 480px; margin-top: 16px;" role="button" tabindex="0" aria-label="Perbesar lampiran" title="Klik untuk memperbesar gambar">
                            <img src="{{ asset('storage/aspirasi/' . $fotoPath) }}" class="img-fluid" alt="Lampiran Bukti">
                        </div>
        ...
        {{-- Kanan: Aksi & Profil --}}
        <div class="col-lg-4">
            {{-- Card Aksi: Premium Action Card --}}
            <div class="bento-card" style="margin-bottom:22px;">
                <h6>Proses Aspirasi</h6>
                
                @if($aspirasi->status == 'Baru')
                    <form action="{{ route('admin.aspirasi.proses', $aspirasi->id_aspirasi) }}" method="POST" class="mb-2" data-no-loading>
                        @csrf
                ...
                @if($aspirasi->status == 'Selesai')
                    <button class="admin-btn btn-feedback" data-bs-toggle="modal" data-bs-target="#umpanBalikModal">
                        ...
                        {{ $aspirasi->umpan_balik ? 'Ubah Tanggapan' : 'Tambah Tanggapan' }}
                    </button>
                @endif
            </div>

            {{-- Card Pelapor: Premium Data Card --}}
            <div class="bento-card">
                <h6>Data Pelapor</h6>
                ...
                <div class="meta-tile">
                    ...
                    <div>
                        ...
                        <span class="fw-bold">{{ $aspirasi->display_email ?? ($aspirasi->email_siswa ?? '-') }}</span>
                    </div>
                </div>
                ...
                <div class="meta-tile">
                    ...
                    <div>
                        ...
                        <span class="fw-bold">{{ $aspirasi->display_nis ?? $aspirasi->nis ?? '-' }}</span>
                    </div>
                </div>
                ...
                <div class="meta-tile">
                    ...
                    <div>
                        ...
                        <span class="fw-bold">{{ $aspirasi->lokasi ?? 'N/A' }}</span>
                    </div>
                </div>
            </div>
        </div>
...
{{-- MODAL ZOOM: Premium Glass Morphism --}}
@if(!empty($fotoPath))
    ...
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content bg-transparent border-0" style="box-shadow: 0 25px 50px rgba(0,0,0,0.2);">
            <div class="modal-body p-0">
                ...
                <img src="{{ asset('storage/aspirasi/' . $fotoPath) }}" class="img-fluid rounded-3" style="width: 100%; object-fit: contain; max-height: 85vh;">
            </div>
        </div>
    </div>
</div>
...
{{-- MODAL UMPAN BALIK: Premium Glass Design --}}
<div class="modal fade" id="umpanBalikModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <form action="{{ route('admin.aspirasi.umpan-balik', $aspirasi->id_aspirasi) }}" method="POST" data-no-loading>
                @csrf
                <div class="modal-body">
                    <label class="small fw-bold text-uppercase mb-3">Pesan Tanggapan</label>
                    <textarea name="umpan_balik" class="form-control" rows="6" required maxlength="2000" placeholder="Jelaskan langkah penyelesaian aspirasi, tindakan yang telah diambil, dan hasil akhir untuk siswa...">{{ $aspirasi->umpan_balik }}</textarea>
...
<div class="modal fade" id="tolakAspirasiModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <form action="{{ route('admin.aspirasi.tolak', $aspirasi->id_aspirasi) }}" method="POST" data-no-loading>
                @csrf
                <div class="modal-body">
                    <label class="small fw-bold text-uppercase mb-3">Alasan Penolakan</label>
                    <textarea name="tolak_alasan" class="form-control" rows="5" required maxlength="2000" placeholder="Jelaskan alasan penolakan aspirasi dan rekomendasi selanjutnya...">{{ old('tolak_alasan') }}</textarea>
