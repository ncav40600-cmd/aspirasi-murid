...
@section('title', 'Admin - Panel Kendali ')
...
<div class="zenith-wrapper">
    ...
    <div class="row g-3 mb-5">
        ...
        <div class="col-xl col-lg-4 col-md-6 stagger-1" style="animation-delay: {{ $index * 0.1 }}s">
            <div class="stat-card">
                <div class="stat-icon-box" style="background: {{ $stat[4] }}; color: {{ $stat[2] }}">
                    <i class="fas {{ $stat[3] }}"></i>
                </div>
                <div class="val-text" style="color: {{ $stat[2] }}">{{ number_format($stat[1]) }}</div>
    ...
    <div class="data-panel p-0 stagger-2 overflow-hidden">
        <div class="p-4 px-5 d-flex justify-content-between align-items-center border-bottom">
            ...
            <a href="{{ route('admin.aspirasi.index') }}" class="btn btn-dark btn-sm rounded-pill px-4 fw-800 shadow-sm" style="font-size: 0.7rem;">
        ...
        <div class="table-responsive p-4">
            <table class="table table-custom align-middle mb-0">
                ...
                <tbody>
                @forelse($aspirasi_terbaru as $item)
                    <tr>
                        <td class="ps-4 fw-800 text-primary">#{{ $item->id_aspirasi }}</td>
                        <td>
                            <div class="d-flex align-items-center">
                                ...
                                <div>
                                    ...
                                    <br>
                                    <small class="text-muted fw-600" style="font-family: monospace;">{{ \Carbon\Carbon::parse($item->created_at)->format('d/m/Y H:i') }}</small>
                                </div>
                                @else
                                <div class="rounded-circle me-3 border shadow-sm d-flex align-items-center justify-content-center fw-900 bg-white" style="width: 35px; height: 35px; font-size: 10px; font-family: 'Outfit';">
                                    {{ substr($item->display_name ?? $item->full_name, 0, 1) }}
                                </div>
                                <div>
                                    <span class="fw-700">{{ $item->display_name ?? $item->full_name }}</span>
                                    <br>
                                    <small class="text-muted fw-600" style="font-family: monospace;">NIS: {{ $item->display_nis ?? $item->nis }}</small>
                                </div>
                                @endif
                            </div>
                        </td>
                        <td><span class="text-muted fw-bold small">{{ $item->nama_kategori }}</span></td>
                        <td>
                            <span class="status-pill {{ match($item->status) { 'Baru' => 'sp-baru', 'Diproses' => 'sp-proses', 'Selesai' => 'sp-selesai', 'Ditolak' => 'sp-void', default => 'bg-secondary' } }}">
                                ● {{ strtoupper($item->status == 'Diproses' ? 'DALAM PROSES' : $item->status) }}
                            </span>
                        </td>
                        <td class="text-end pe-4">
                            <a href="{{ route('admin.aspirasi.show', $item->id_aspirasi) }}" class="btn btn-light btn-sm rounded-pill border fw-800" style="font-size: 10px;">
    ...
    document.addEventListener("DOMContentLoaded", function() {
        ...
        new Chart(document.getElementById('statusChart'), {
            ...
            data: {
                ...
                datasets: [{
                    data: [{{ $baru }}, {{ $diproses }}, {{ $selesai }}, {{ $ditolak }}],
